...
abstract class FlinkTableSourceFailOverITCase {
    ...
    @TempDir
    public static File checkpointDir;

    @TempDir
    public static File savepointDir;
    ...
    private StreamTableEnvironment initTableEnvironment(@Nullable String savepointPath) {
        ...
        StreamTableEnvironment tEnv = StreamTableEnvironment.create(execEnv, EnvironmentSettings.inStreamingMode());
    ...
    void testRestore() throws Exception {
        ...
        try {
            ...
            tEnv.executeSql("create table test_partitioned ("
                    + "a int, b varchar"
                    + ") partitioned by (b) "
                    + "with ("
            ...
            TableResult insertResult = tEnv.executeSql("insert into result_table select * from test_partitioned");
            ...
            String savepointPath = insertResult
                    .getJobClient()
                    .get()
                    .stopWithSavepoint(false, savepointDir.getAbsolutePath(), SavepointFormatType.CANONICAL)
                    .get();

            tEnv = initTableEnvironment(savepointPath);
            insertResult = tEnv.executeSql("insert into result_table select * from test_partitioned");
