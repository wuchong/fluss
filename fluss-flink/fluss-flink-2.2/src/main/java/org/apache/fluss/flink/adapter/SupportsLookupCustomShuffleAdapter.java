...
public interface SupportsLookupCustomShuffleAdapter extends SupportsLookupCustomShuffle {
    ...
    @Override
    default Optional<InputDataPartitioner> getPartitioner() {
        Optional<InputDataPartitionerAdapter> partitionerAdapter = getPartitionerAdapter();
        return partitionerAdapter.isPresent()
                ? Optional.of(partitionerAdapter.get())
                : Optional.empty();
    }
