...
public class DataTestUtils {
    ...
    private static IndexedRow genIndexed(RowType rowType, Object[] data) {
        ...
        for (int i = 0; i < dataTypes.length; i++) {
            rowEncoder.encodeField(i, data[i] instanceof String ? BinaryString.fromString((String) data[i]) : data[i]);
        }
    ...
    public static MemoryLogRecords genMemoryLogRecordsByObject(List<Object[]> objects) throws Exception {
        return createRecordsWithoutBaseLogOffset(
                DATA1_ROW_TYPE, DEFAULT_SCHEMA_ID, 0, System.currentTimeMillis(), objects, LogFormat.ARROW);
    }

    public static MemoryLogRecords genMemoryLogRecordsWithWriterId(
            List<Object[]> objects, long writerId, int batchSequence, long baseOffset) throws Exception {
    ...
    public static MemoryLogRecords genMemoryLogRecordsWithBaseOffset(long offsetBase, List<Object[]> objects)
            throws Exception {
        return createRecordsWithoutBaseLogOffset(
    ...
    public static MemoryLogRecords genLogRecordsWithBaseOffsetAndTimestamp(
        ...
        return createRecordsWithoutBaseLogOffset(
                DATA1_ROW_TYPE, DEFAULT_SCHEMA_ID, offsetBase, maxTimestamp, objects, LogFormat.ARROW);
    }
    ...
    public static KvRecordBatch genKvRecordBatch(
            RowType keyType, RowType valueType, List<Tuple2<Object[], Object[]>> keyAndValues) throws Exception {
        return genKvRecordBatchWithWriterId(keyAndValues, keyType, valueType, NO_WRITER_ID, NO_BATCH_SEQUENCE);
    }
    ...
    public static KvRecordBatch genKvRecordBatchWithWriterId(
        ...
        KvRecordTestUtils.KvRecordFactory kvRecordFactory = KvRecordTestUtils.KvRecordFactory.of(valueType);
        List<KvRecord> records = new ArrayList<>();
        for (Tuple2<Object[], Object[]> keyAndValue : keyAndValues) {
            records.add(kvRecordFactory.ofRecord(keyEncoder.encodeKey(row(keyAndValue.f0)), keyAndValue.f1));
        }
    ...
    public static void genRemoteLogSegmentFile(
            ...
            long baseOffset)
            throws Exception {
        FsPath remoteLogTabletDir = remoteLogTabletDir(remoteLogDir(conf), physicalTablePath, tableBucket);
        FsPath remoteLogSegmentDir = remoteLogSegmentDir(remoteLogTabletDir, remoteLogSegment.remoteLogSegmentId());
        genLogFile(DATA1_ROW_TYPE, new File(remoteLogSegmentDir.toString()), DATA1, baseOffset, LogFormat.ARROW);
    }

    public static File genLogFile(
            RowType rowType, File segmentDir, List<Object[]> objects, long baseOffset, LogFormat logFormat)
            throws Exception {
        ...
        fileLogRecords.append(createRecordsWithoutBaseLogOffset(
                rowType, DEFAULT_SCHEMA_ID, baseOffset, System.currentTimeMillis(), objects, logFormat));
    ...
    public static List<KvRecord> genKvRecords(Object[]... values) {
        KvRecordTestUtils.PKBasedKvRecordFactory kvRecordFactory = KvRecordTestUtils.PKBasedKvRecordFactory.of(
    ...
    public static MemoryLogRecords createMemoryLogRecords(
        ...
        if (logFormat == LogFormat.ARROW) {
            List<InternalRow> rows = objects.stream().map(DataTestUtils::row).collect(Collectors.toList());
            ...
            return createIndexedMemoryLogRecords(
                    ...
                    batchSequence,
                    changeTypes,
                    objects.stream().map(object -> indexedRow(rowType, object)).collect(Collectors.toList()));
        }
    }
    ...
    private static MemoryLogRecords createArrowMemoryLogRecords(
        ...
        try (BufferAllocator allocator = new RootAllocator(Integer.MAX_VALUE);
            ...
            ArrowWriter writer =
                    provider.getOrCreateWriter(1L, schemaId, Integer.MAX_VALUE, rowType, arrowCompressionInfo);
            MemoryLogRecordsArrowBuilder builder = MemoryLogRecordsArrowBuilder.builder(
                    baseLogOffset,
                    schemaId,
                    writer,
    ...
    public static void assertMemoryRecordsEquals(RowType rowType, LogRecords records, List<List<Object[]>> expected) {
        ...
        for (List<Object[]> expectedRecord : expected) {
            List<Tuple2<ChangeType, Object[]>> expectedFieldAndRowKind = expectedRecord.stream()
    ...
    public static void assertMemoryRecordsEqualsWithRowKind(
            RowType rowType, LogRecords records, List<List<Tuple2<ChangeType, Object[]>>> expected) {
        ...
        for (List<Tuple2<ChangeType, Object[]>> expectedRecord : expected) {
            ...
            try (LogRecordReadContext readContext = createArrowReadContext(rowType, DEFAULT_SCHEMA_ID);
                ...
                for (Tuple2<ChangeType, Object[]> expectedFieldAndRowKind : expectedRecord) {
                    ...
                    assertLogRecordsEqualsWithRowKind(rowType, logIterator.next(), expectedFieldAndRowKind);
                }
    ...
    public static void assertLogRecordBatchEqualsWithRowKind(
            RowType rowType, LogRecordBatch logRecordBatch, List<Tuple2<ChangeType, Object[]>> expected) {
        try (LogRecordReadContext readContext = createArrowReadContext(rowType, DEFAULT_SCHEMA_ID);
    ...
    private static void assertLogRecordsEqualsWithRowKind(
            RowType rowType, LogRecord logRecord, Tuple2<ChangeType, Object[]> expectedFieldAndRowKind) {
        DataType[] dataTypes = rowType.getChildren().toArray(new DataType[0]);
        ...
        assertRowValueEquals(fieldGetter, dataTypes, logRecord.getRow(), expectedFieldAndRowKind.f1);
    }

    public static void assertLogRecordsEquals(RowType rowType, LogRecords logRecords, List<Object[]> expectedValue) {
        List<Tuple2<ChangeType, Object[]>> expectedValueWithRowKind = expectedValue.stream()
                .map(val -> Tuple2.of(ChangeType.APPEND_ONLY, val))
                .collect(Collectors.toList());
    ...
    public static void assertLogRecordsEqualsWithRowKind(
            RowType rowType, LogRecords logRecords, List<Tuple2<ChangeType, Object[]>> expectedValue) {
        DataType[] dataTypes = rowType.getChildren().toArray(new DataType[0]);
        InternalRow.FieldGetter[] fieldGetter = new InternalRow.FieldGetter[dataTypes.length];
        for (int i = 0; i < dataTypes.length; i++) {
            fieldGetter[i] = InternalRow.createFieldGetter(dataTypes[i], i);
    ...
    private static void assertRowValueEquals(
            InternalRow.FieldGetter[] fieldGetter, DataType[] dataTypes, InternalRow row, Object[] expectVal) {
        for (int i = 0; i < dataTypes.length; i++) {
