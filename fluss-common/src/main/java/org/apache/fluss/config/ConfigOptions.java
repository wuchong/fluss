...
public class ConfigOptions {
    ...
    // ------------------------------------------------------------------------
    public static final ConfigOption<Integer> DEFAULT_BUCKET_NUMBER = key("default.bucket.number")
            .intType()
            .defaultValue(1)
            .withDescription("The default number of buckets for a table in Fluss cluster. It's "
    ...
    public static final ConfigOption<Integer> DEFAULT_REPLICATION_FACTOR = key("default.replication.factor")
            .intType()
            .defaultValue(1)
            .withDescription("The default replication factor for the log of a table in Fluss cluster. It's "
                    + "a cluster-level parameter, "
                    + "and all the tables without specifying replication factor in the cluster will use the value "
                    + "as replication factor.");

    public static final ConfigOption<String> REMOTE_DATA_DIR = key("remote.data.dir")
            .stringType()
            .noDefaultValue()
            .withDescription(
    ...
    public static final ConfigOption<MemorySize> REMOTE_FS_WRITE_BUFFER_SIZE = key("remote.fs.write-buffer-size")
            .memoryType()
    ...
    public static final ConfigOption<List<String>> PLUGIN_ALWAYS_PARENT_FIRST_LOADER_PATTERNS =
            key("plugin.classloader.parent-first-patterns.default")
                    .stringType()
                    .asList()
                    .defaultValues(ArrayUtils.concat(
                            new String[] {"java.", "org.apache.fluss.", "javax.annotation."},
    ...
    public static final ConfigOption<Duration> AUTO_PARTITION_CHECK_INTERVAL = key("auto-partition.check.interval")
            .durationType()
            .defaultValue(Duration.ofMinutes(10))
            .withDescription("The interval of auto partition check. " + "The default value is 10 minutes.");

    public static final ConfigOption<Integer> MAX_PARTITION_NUM = key("max.partition.num")
            .intType()
            .defaultValue(1000)
            .withDescription("Limits the maximum number of partitions that can be created for a partitioned table "
    ...
    public static final ConfigOption<Boolean> AUTHORIZER_ENABLED = key("authorizer.enabled")
            .booleanType()
            .defaultValue(false)
            .withDescription("Specifies whether to enable the authorization feature. "
    ...
    public static final ConfigOption<String> AUTHORIZER_TYPE = key("authorizer.type")
            .stringType()
            .defaultValue("default")
            .withDescription("Specifies the type of authorizer to be used for access control. "
    ...
    public static final ConfigOption<String> SUPER_USERS = key("super.users")
            .stringType()
            .noDefaultValue()
            .withDescription(
    ...
    public static final ConfigOption<Integer> MAX_BUCKET_NUM = key("max.bucket.num")
            .intType()
            .defaultValue(128000)
            .withDescription(
     ...
     */
    public static final ConfigOption<String> BIND_LISTENERS = key("bind.listeners")
            .stringType()
            .noDefaultValue()
            .withDescription("The network address and port to which the server binds for accepting connections. "
     ...
     */
    public static final ConfigOption<String> ADVERTISED_LISTENERS = key("advertised.listeners")
            .stringType()
            .noDefaultValue()
            .withDescription("The externally advertised address and port for client connections. "
    ...
    public static final ConfigOption<String> INTERNAL_LISTENER_NAME = key("internal.listener.name")
            .stringType()
     ...
     */
    @Deprecated
    public static final ConfigOption<String> COORDINATOR_HOST = key("coordinator.host")
            .stringType()
            .noDefaultValue()
            .withDescription(
     ...
     */
    @Deprecated
    public static final ConfigOption<String> COORDINATOR_PORT = key("coordinator.port")
            .stringType()
            .defaultValue("9123")
            .withDescription(
    ...
    public static final ConfigOption<Integer> COORDINATOR_IO_POOL_SIZE = key("coordinator.io-pool.size")
            .intType()
            .defaultValue(10)
            .withDescription("The size of the IO thread pool to run blocking operations for coordinator server. "
     ...
     */
    @Deprecated
    public static final ConfigOption<String> TABLET_SERVER_HOST = key("tablet-server.host")
            .stringType()
            .noDefaultValue()
            .withDescription(
     ...
     */
    @Deprecated
    public static final ConfigOption<String> TABLET_SERVER_PORT = key("tablet-server.port")
            .stringType()
            .defaultValue("0")
            .withDescription(
    ...
    public static final ConfigOption<Map<String, String>> SERVER_SECURITY_PROTOCOL_MAP = key("security.protocol.map")
            .mapType()
    ...
    public static final ConfigOption<Integer> TABLET_SERVER_ID =
            key("tablet-server.id").intType().noDefaultValue().withDescription("The id for the tablet server.");

    public static final ConfigOption<String> TABLET_SERVER_RACK = key("tablet-server.rack")
            .stringType()
            .noDefaultValue()
            .withDescription("The rack for the tabletServer. This will be used in rack aware bucket assignment "
                    + "for fault tolerance. Examples: `RACK1`, `cn-hangzhou-server10`");

    public static final ConfigOption<String> DATA_DIR = key("data.dir")
            .stringType()
            .defaultValue("/tmp/fluss-data")
            .withDescription("This configuration controls the directory where fluss will store its data. "
                    + "The default value is /tmp/fluss-data");

    public static final ConfigOption<Duration> WRITER_ID_EXPIRATION_TIME = key("server.writer-id.expiration-time")
            .durationType()
            .defaultValue(Duration.ofDays(7))
            .withDescription("The time that the tablet server will wait without receiving any write request from "
    ...
    public static final ConfigOption<Duration> WRITER_ID_EXPIRATION_CHECK_INTERVAL =
            key("server.writer-id.expiration-check-interval")
                    .durationType()
                    .defaultValue(Duration.ofMinutes(10))
                    .withDescription("The interval at which to remove writer ids that have expired due to "
                            + WRITER_ID_EXPIRATION_TIME.key()
                            + " passing. The default value is 10 minutes.");

    public static final ConfigOption<Integer> BACKGROUND_THREADS = key("server.background.threads")
            .intType()
            .defaultValue(10)
            .withDescription("The number of threads to use for various background processing tasks.");

    public static final ConfigOption<MemorySize> SERVER_BUFFER_MEMORY_SIZE = key("server.buffer.memory-size")
            .memoryType()
            .defaultValue(MemorySize.parse("256mb"))
            .withDescription("The total bytes of memory the server can use, e.g, buffer write-ahead-log rows.");

    public static final ConfigOption<MemorySize> SERVER_BUFFER_PAGE_SIZE = key("server.buffer.page-size")
            .memoryType()
            .defaultValue(MemorySize.parse("128kb"))
            .withDescription("Size of every page in memory buffers (`" + SERVER_BUFFER_MEMORY_SIZE.key() + "`).");
    ...
    public static final ConfigOption<Duration> SERVER_BUFFER_POOL_WAIT_TIMEOUT = key("server.buffer.wait-timeout")
            .durationType()
    ...
    public static final ConfigOption<String> ZOOKEEPER_ADDRESS = key("zookeeper.address")
            .stringType()
            .noDefaultValue()
            .withDescription("The ZooKeeper address to use, when running Fluss with ZooKeeper.");

    /** The root path under which Fluss stores its entries in ZooKeeper. */
    public static final ConfigOption<String> ZOOKEEPER_ROOT = key("zookeeper.path.root")
            .stringType()
            .defaultValue("/fluss")
            .withDescription("The root path under which Fluss stores its entries in ZooKeeper.");

    // ------------------------------------------------------------------------
    //  ZooKeeper Client Settings
    // ------------------------------------------------------------------------

    public static final ConfigOption<Duration> ZOOKEEPER_SESSION_TIMEOUT = key("zookeeper.client.session-timeout")
            .durationType()
    ...
    public static final ConfigOption<Duration> ZOOKEEPER_CONNECTION_TIMEOUT = key("zookeeper.client.connection-timeout")
            .durationType()
    ...
    public static final ConfigOption<Duration> ZOOKEEPER_RETRY_WAIT = key("zookeeper.client.retry-wait")
            .durationType()
    ...
    public static final ConfigOption<Boolean> ZOOKEEPER_ENSEMBLE_TRACKING = key("zookeeper.client.ensemble-tracker")
            .booleanType()
            .defaultValue(true)
            .withDescription(
    ...
    public static final ConfigOption<String> ZOOKEEPER_CONFIG_PATH = key("zookeeper.client.config-path")
            .stringType()
            .noDefaultValue()
            .withDescription("The file path from which the ZooKeeper client reads its configuration. "
    ...
    public static final ConfigOption<MemorySize> LOG_SEGMENT_FILE_SIZE = key("log.segment.file-size")
            .memoryType()
            .defaultValue(MemorySize.parse("1024m"))
            .withDescription("This configuration controls the segment file size for the log. "
    ...
    public static final ConfigOption<MemorySize> LOG_INDEX_FILE_SIZE = key("log.index.file-size")
            .memoryType()
            .defaultValue(MemorySize.parse("10m"))
            .withDescription("This configuration controls the size of the index that maps offsets to file positions. "
                    + "We preallocate this index file and shrink it only after log rolls. You generally "
                    + "should not need to change this setting.");

    public static final ConfigOption<MemorySize> LOG_INDEX_INTERVAL_SIZE = key("log.index.interval-size")
            .memoryType()
            .defaultValue(MemorySize.parse("4k"))
            .withDescription("This setting controls how frequently fluss adds an index entry to its offset index. "
    ...
    public static final ConfigOption<Boolean> LOG_FILE_PREALLOCATE = key("log.file-preallocate")
            .booleanType()
            .defaultValue(false)
            .withDescription("True if we should preallocate the file on disk when creating a new log segment.");

    public static final ConfigOption<Long> LOG_FLUSH_INTERVAL_MESSAGES = key("log.flush.interval-messages")
            .longType()
            .defaultValue(Long.MAX_VALUE)
            .withDescription("This setting allows specifying an interval at which we will force a "
    ...
    public static final ConfigOption<Duration> LOG_REPLICA_HIGH_WATERMARK_CHECKPOINT_INTERVAL =
            key("log.replica.high-watermark.checkpoint-interval")
                    ...
                    .withDescription("The frequency with which the high watermark is saved out to disk. "
                            + "The default setting is 5 seconds.");

    public static final ConfigOption<Duration> LOG_REPLICA_MAX_LAG_TIME = key("log.replica.max-lag-time")
            .durationType()
            .defaultValue(Duration.ofSeconds(30))
            .withDescription("If a follower replica hasn't sent any fetch log requests or hasn't "
    ...
    public static final ConfigOption<Integer> LOG_REPLICA_WRITE_OPERATION_PURGE_NUMBER =
            key("log.replica.write-operation-purge-number")
                    .intType()
                    .defaultValue(1000)
                    .withDescription("The purge number (in number of requests) of the write operation manager, "
    ...
    public static final ConfigOption<Integer> LOG_REPLICA_FETCH_OPERATION_PURGE_NUMBER =
            key("log.replica.fetch-operation-purge-number")
                    .intType()
                    .defaultValue(1000)
                    .withDescription("The purge number (in number of requests) of the fetch log operation manager, "
                            + "the default value is 1000.");

    public static final ConfigOption<Integer> LOG_REPLICA_FETCHER_NUMBER = key("log.replica.fetcher-number")
            .intType()
            .defaultValue(1)
            .withDescription("Number of fetcher threads used to replicate log records from each source tablet server. "
    ...
    public static final ConfigOption<MemorySize> LOG_REPLICA_FETCH_MAX_BYTES = key("log.replica.fetch.max-bytes")
            .memoryType()
            .defaultValue(MemorySize.parse("16mb"))
            .withDescription("The maximum amount of data the server should return for a fetch request from follower. "
    ...
    public static final ConfigOption<Duration> LOG_REPLICA_FETCH_WAIT_MAX_TIME = key("log.replica.fetch.wait-max-time")
            .durationType()
            .defaultValue(Duration.ofMillis(500))
            .withDescription("The maximum time to wait for enough bytes to be available for a fetch log request "
    ...
    public static final ConfigOption<MemorySize> LOG_REPLICA_FETCH_MIN_BYTES = key("log.replica.fetch.min-bytes")
            .memoryType()
            .defaultValue(MemorySize.parse("1b"))
            .withDescription("The minimum bytes expected for each fetch log request from follower to response. "
    ...
    public static final ConfigOption<Duration> REMOTE_LOG_TASK_INTERVAL_DURATION =
            key("remote.log.task-interval-duration")
                    ...
                    .withDescription("Interval at which remote log manager runs the scheduled tasks like "
    ...
    public static final ConfigOption<MemorySize> REMOTE_LOG_INDEX_FILE_CACHE_SIZE =
            key("remote.log.index-file-cache-size")
                    ...
                    .withDescription("The total size of the space allocated to store index files fetched "
    ...
    public static final ConfigOption<Integer> REMOTE_LOG_MANAGER_THREAD_POOL_SIZE =
            key("remote.log-manager.thread-pool-size")
                    .intType()
                    .defaultValue(4)
                    .withDescription("Size of the thread pool used in scheduling tasks to copy segments, "
    ...
    public static final ConfigOption<Integer> REMOTE_LOG_DATA_TRANSFER_THREAD_NUM =
            key("remote.log.data-transfer-thread-num")
                    .intType()
                    .defaultValue(4)
                    .withDescription("The number of threads the server uses to transfer (download and upload) "
    ...
    public static final ConfigOption<Integer> NETTY_SERVER_NUM_NETWORK_THREADS = key("netty.server.num-network-threads")
            .intType()
            .defaultValue(3)
            .withDescription("The number of threads that the server uses for receiving requests "
                    + "from the network and sending responses to the network.");

    public static final ConfigOption<Integer> NETTY_SERVER_NUM_WORKER_THREADS = key("netty.server.num-worker-threads")
            .intType()
            .defaultValue(8)
            .withDescription("The number of threads that the server uses for processing requests, "
                    + "which may include disk and remote I/O.");

    public static final ConfigOption<Integer> NETTY_SERVER_MAX_QUEUED_REQUESTS = key("netty.server.max-queued-requests")
            .intType()
            .defaultValue(500)
            .withDescription(
                    "The number of queued requests allowed for worker threads, before blocking the I/O threads.");

    public static final ConfigOption<Duration> NETTY_CONNECTION_MAX_IDLE_TIME = key("netty.connection.max-idle-time")
            .durationType()
            .defaultValue(Duration.ofMinutes(10))
            .withDescription("Close idle connections after the given time specified by this config.");

    public static final ConfigOption<Integer> NETTY_CLIENT_NUM_NETWORK_THREADS = key("netty.client.num-network-threads")
            .intType()
            .defaultValue(1)
            .withDescription("The number of threads that the client uses for sending requests to the "
    ...
    public static final ConfigOption<String> CLIENT_ID = key("client.id")
            .stringType()
            .defaultValue("")
            .withDescription("An id string to pass to the server when making requests. The purpose of this is "
    ...
    public static final ConfigOption<Duration> CLIENT_CONNECT_TIMEOUT = key("client.connect-timeout")
            .durationType()
            .defaultValue(Duration.ofSeconds(120))
            .withDescription("The Netty client connect timeout.");

    public static final ConfigOption<List<String>> BOOTSTRAP_SERVERS = key("bootstrap.servers")
            .stringType()
            .asList()
            .noDefaultValue()
            .withDescription(
    ...
    public static final ConfigOption<MemorySize> CLIENT_WRITER_BUFFER_MEMORY_SIZE =
            key("client.writer.buffer.memory-size")
                    ...
                    .withDescription("The total bytes of memory the writer can use to buffer internal rows.");

    public static final ConfigOption<MemorySize> CLIENT_WRITER_BUFFER_PAGE_SIZE = key("client.writer.buffer.page-size")
            .memoryType()
            .defaultValue(MemorySize.parse("128kb"))
            .withDescription(
                    "Size of every page in memory buffers (`" + CLIENT_WRITER_BUFFER_MEMORY_SIZE.key() + "`).");
    ...
    public static final ConfigOption<MemorySize> CLIENT_WRITER_BATCH_SIZE = key("client.writer.batch-size")
            .memoryType()
            .defaultValue(MemorySize.parse("2mb"))
            .withDescription("The writer or walBuilder will attempt to batch records together into one batch for"
    ...
    public static final ConfigOption<Duration> CLIENT_WRITER_BATCH_TIMEOUT = key("client.writer.batch-timeout")
            .durationType()
            .defaultValue(Duration.ofMillis(100))
            .withDescription("The writer groups ay rows that arrive in between request sends into a single batched"
    ...
    public static final ConfigOption<String> CLIENT_WRITER_ACKS = key("client.writer.acks")
            .stringType()
            .defaultValue("all")
            .withDescription("The number of acknowledgments the writer requires the leader to have received before"
    ...
    public static final ConfigOption<MemorySize> CLIENT_WRITER_REQUEST_MAX_SIZE = key("client.writer.request-max-size")
            .memoryType()
            .defaultValue(MemorySize.parse("10mb"))
            .withDescription("The maximum size of a request in bytes. This setting will limit the number of "
    ...
    public static final ConfigOption<Integer> CLIENT_WRITER_RETRIES = key("client.writer.retries")
            .intType()
            .defaultValue(Integer.MAX_VALUE)
            .withDescription("Setting a value greater than zero will cause the client to resend any record whose "
                    + "send fails with a potentially transient error.");

    public static final ConfigOption<Boolean> CLIENT_WRITER_ENABLE_IDEMPOTENCE = key("client.writer.enable-idempotence")
            .booleanType()
            .defaultValue(true)
            .withDescription("Enable idempotence for the writer. When idempotence is enabled, the writer "
    ...
    public static final ConfigOption<Boolean> CLIENT_WRITER_DYNAMIC_CREATE_PARTITION_ENABLED =
            key("client.writer.dynamic-create-partition.enabled")
                    .booleanType()
                    .defaultValue(true)
                    .withDescription("Whether enable dynamic create partition for client writer. Enable by default."
    ...
    public static final ConfigOption<Duration> CLIENT_REQUEST_TIMEOUT = key("client.request-timeout")
            .durationType()
            .defaultValue(Duration.ofSeconds(30))
            .withDescription("The timeout for a request to complete. If user set the write ack to -1, "
                    + "this timeout is the max time that delayed write try to complete. "
                    + "The default setting is 30 seconds.");

    public static final ConfigOption<Boolean> CLIENT_SCANNER_LOG_CHECK_CRC = key("client.scanner.log.check-crc")
            .booleanType()
            .defaultValue(true)
            .withDescription(
    ...
    public static final ConfigOption<String> CLIENT_SECURITY_PROTOCOL = key("client.security.protocol")
            .stringType()
            .defaultValue("PLAINTEXT")
            .withDescription("The authentication protocol used to authenticate the client.");
    ...
    public static final ConfigOption<Duration> CLIENT_SCANNER_LOG_FETCH_WAIT_MAX_TIME =
            key("client.scanner.log.fetch.wait-max-time")
                    .durationType()
                    .defaultValue(Duration.ofMillis(500))
                    .withDescription("The maximum time to wait for enough bytes to be available for a fetch log "
    ...
    public static final ConfigOption<MemorySize> CLIENT_SCANNER_LOG_FETCH_MIN_BYTES =
            key("client.scanner.log.fetch.min-bytes")
                    .memoryType()
                    .defaultValue(MemorySize.parse("1b"))
                    .withDescription("The minimum bytes expected for each fetch log request from client to response. "
                            + "If not enough bytes, wait up to "
                            + CLIENT_SCANNER_LOG_FETCH_WAIT_MAX_TIME.key()
                            + " time to return.");

    public static final ConfigOption<Integer> CLIENT_LOOKUP_QUEUE_SIZE = key("client.lookup.queue-size")
            .intType()
            .defaultValue(25600)
            .withDescription("The maximum number of pending lookup operations.");

    public static final ConfigOption<Integer> CLIENT_LOOKUP_MAX_BATCH_SIZE = key("client.lookup.max-batch-size")
            .intType()
            .defaultValue(128)
            .withDescription("The maximum batch size of merging lookup operations to one lookup request.");

    public static final ConfigOption<Integer> CLIENT_LOOKUP_MAX_INFLIGHT_SIZE =
            key("client.lookup.max-inflight-requests")
                    .intType()
                    .defaultValue(128)
                    .withDescription("The maximum number of unacknowledged lookup requests for lookup operations.");

    public static final ConfigOption<Duration> CLIENT_LOOKUP_BATCH_TIMEOUT = key("client.lookup.batch-timeout")
            .durationType()
            .defaultValue(Duration.ofMillis(100))
            .withDescription("The maximum time to wait for the lookup batch to full, if this timeout is reached, "
    ...
    public static final ConfigOption<Integer> CLIENT_SCANNER_REMOTE_LOG_PREFETCH_NUM =
            key("client.scanner.remote-log.prefetch-num")
                    .intType()
                    .defaultValue(4)
                    .withDescription("The number of remote log segments to keep in local temp file for LogScanner, "
                            + "which download from remote storage. The default setting is 4.");

    public static final ConfigOption<String> CLIENT_SCANNER_IO_TMP_DIR = key("client.scanner.io.tmpdir")
            .stringType()
            .defaultValue(System.getProperty("java.io.tmpdir") + "/fluss")
            .withDescription("Local directory that is used by client for"
    ...
    public static final ConfigOption<Integer> REMOTE_FILE_DOWNLOAD_THREAD_NUM =
            key("client.remote-file.download-thread-num")
                    .intType()
                    .defaultValue(3)
                    .withDescription("The number of threads the client uses to download remote files.");
    ...
    public static final ConfigOption<Duration> FILESYSTEM_SECURITY_TOKEN_RENEWAL_RETRY_BACKOFF =
            key("client.filesystem.security.token.renewal.backoff")
                    ...
                    .withDescription("The time period how long to wait before retrying to obtain new security tokens "
    ...
    public static final ConfigOption<Boolean> CLIENT_METRICS_ENABLED = key("client.metrics.enabled")
            .booleanType()
            .defaultValue(false)
            .withDescription("Enable metrics for client. When metrics is enabled, the client "
                    + "will collect metrics and report by the JMX metrics reporter.");

    public static final ConfigOption<String> CLIENT_SASL_MECHANISM = key("client.security.sasl.mechanism")
            .stringType()
            .defaultValue("PLAIN")
            .withDescription("SASL mechanism to use for authentication.Currently, we only support plain.");

    public static final ConfigOption<String> CLIENT_SASL_JAAS_CONFIG = key("client.security.sasl.jaas.config")
            .stringType()
            .noDefaultValue()
            .withDescription(
    ...
    public static final ConfigOption<String> CLIENT_SASL_JAAS_USERNAME = key("client.security.sasl.username")
            .stringType()
            .noDefaultValue()
            .withDescription(
    ...
    public static final ConfigOption<String> CLIENT_SASL_JAAS_PASSWORD = key("client.security.sasl.password")
            .stringType()
            .noDefaultValue()
            .withDescription(
    ...
    // ------------------------------------------------------------------------
    public static final ConfigOption<Integer> TABLE_REPLICATION_FACTOR = key("table.replication.factor")
            .intType()
            .noDefaultValue()
            .withDescription("The replication factor for the log of the new table. When it's not set, Fluss "
    ...
    public static final ConfigOption<LogFormat> TABLE_LOG_FORMAT = key("table.log.format")
            .enumType(LogFormat.class)
            .defaultValue(LogFormat.ARROW)
            .withDescription("The format of the log records in log store. The default value is `arrow`. "
    ...
    public static final ConfigOption<ArrowCompressionType> TABLE_LOG_ARROW_COMPRESSION_TYPE =
            key("table.log.arrow.compression.type")
                    ...
                    .withDescription("The compression type of the log records if the log format is set to `ARROW`. "
    ...
    public static final ConfigOption<KvFormat> TABLE_KV_FORMAT = key("table.kv.format")
            .enumType(KvFormat.class)
            .defaultValue(KvFormat.COMPACTED)
            .withDescription("The format of the kv records in kv store. The default value is `compacted`. "
                    + "The supported formats are `compacted` and `indexed`.");

    public static final ConfigOption<Boolean> TABLE_AUTO_PARTITION_ENABLED = key("table.auto-partition.enabled")
            .booleanType()
            .defaultValue(false)
            .withDescription("Whether enable auto partition for the table. Disable by default."
                    + " When auto partition is enabled, the partitions of the table will be created automatically.");

    public static final ConfigOption<String> TABLE_AUTO_PARTITION_KEY = key("table.auto-partition.key")
            .stringType()
            .noDefaultValue()
            .withDescription(
    ...
    public static final ConfigOption<AutoPartitionTimeUnit> TABLE_AUTO_PARTITION_TIME_UNIT =
            key("table.auto-partition.time-unit")
                    ...
                    .withDescription("The time granularity for auto created partitions. "
    ...
    public static final ConfigOption<String> TABLE_AUTO_PARTITION_TIMEZONE = key("table.auto-partition.time-zone")
            .stringType()
    ...
    public static final ConfigOption<Duration> TABLE_LOG_TTL = key("table.log.ttl")
            .durationType()
            .defaultValue(Duration.ofDays(7))
            .withDescription("The time to live for log segments. The configuration controls the maximum time "
                    + "we will retain a log before we will delete old segments to free up "
                    + "space. If set to -1, the log will not be deleted.");

    public static final ConfigOption<Integer> TABLE_TIERED_LOG_LOCAL_SEGMENTS = key("table.log.tiered.local-segments")
            .intType()
            .defaultValue(2)
            .withDescription(
    ...
    public static final ConfigOption<Boolean> TABLE_DATALAKE_ENABLED = key("table.datalake.enabled")
            .booleanType()
            .defaultValue(false)
            .withDescription("Whether enable lakehouse storage for the table. Disabled by default. "
    ...
    public static final ConfigOption<DataLakeFormat> TABLE_DATALAKE_FORMAT = key("table.datalake.format")
    ...
    public static final ConfigOption<Duration> TABLE_DATALAKE_FRESHNESS = key("table.datalake.freshness")
            .durationType()
    ...
    public static final ConfigOption<MergeEngineType> TABLE_MERGE_ENGINE = key("table.merge-engine")
    ...
    public static final ConfigOption<String> TABLE_MERGE_ENGINE_VERSION_COLUMN =
            ...
            key("table.merge-engine.versioned.ver-column")
                    .stringType()
                    .noDefaultValue()
                    .withDescription("The column name of the version column for the `versioned` merge engine. "
                            + "If the merge engine is set to `versioned`, the version column must be set.");

    // ------------------------------------------------------------------------
    //  ConfigOptions for Kv
    // ------------------------------------------------------------------------
    public static final ConfigOption<Duration> KV_SNAPSHOT_INTERVAL = key("kv.snapshot.interval")
            .durationType()
            .defaultValue(Duration.ofMinutes(10))
            .withDescription(
                    "The interval to perform periodic snapshot for kv data. " + "The default setting is 10 minutes.");

    public static final ConfigOption<Integer> KV_SNAPSHOT_SCHEDULER_THREAD_NUM = key("kv.snapshot.scheduler-thread-num")
            .intType()
            .defaultValue(1)
            .withDescription(
                    "The number of threads that the server uses to schedule snapshot kv data for all the replicas in the server.");

    public static final ConfigOption<Integer> KV_SNAPSHOT_TRANSFER_THREAD_NUM = key("kv.snapshot.transfer-thread-num")
            .intType()
            .defaultValue(4)
            .withDescription(
                    "The number of threads the server uses to transfer (download and upload) kv snapshot files.");

    public static final ConfigOption<Integer> KV_MAX_RETAINED_SNAPSHOTS = key("kv.snapshot.num-retained")
            .intType()
            .defaultValue(1)
            .withDescription("The maximum number of completed snapshots to retain.");

    public static final ConfigOption<Integer> KV_MAX_BACKGROUND_THREADS = key("kv.rocksdb.thread.num")
            .intType()
            .defaultValue(2)
            .withDescription(
    ...
    public static final ConfigOption<Integer> KV_MAX_OPEN_FILES = key("kv.rocksdb.files.open")
            .intType()
            .defaultValue(-1)
            .withDescription(
    ...
    public static final ConfigOption<MemorySize> KV_LOG_MAX_FILE_SIZE = key("kv.rocksdb.log.max-file-size")
            .memoryType()
            .defaultValue(MemorySize.parse("25mb"))
            .withDescription("The maximum size of RocksDB's file used for information logging. "
                    + "If the log files becomes larger than this, a new file will be created. "
                    + "If 0, all logs will be written to one log file. "
                    + "The default maximum file size is `25MB`. ");

    public static final ConfigOption<Integer> KV_LOG_FILE_NUM = key("kv.rocksdb.log.file-num")
            .intType()
            .defaultValue(4)
            .withDescription(
                    "The maximum number of files RocksDB should keep for information logging (Default setting: 4).");

    public static final ConfigOption<String> KV_LOG_DIR = key("kv.rocksdb.log.dir")
            .stringType()
            .noDefaultValue()
            .withDescription(
    ...
    public static final ConfigOption<InfoLogLevel> KV_LOG_LEVEL = key("kv.rocksdb.log.level")
    ...
    public static final ConfigOption<MemorySize> KV_WRITE_BATCH_SIZE = key("kv.rocksdb.write-batch-size")
            .memoryType()
            .defaultValue(MemorySize.parse("2mb"))
    ...
    public static final ConfigOption<CompactionStyle> KV_COMPACTION_STYLE = key("kv.rocksdb.compaction.style")
            ...
            .withDescription(String.format(
                    ...
                    LEVEL.name(), FIFO.name(), UNIVERSAL.name(), NONE.name(), LEVEL.name()));
    ...
    public static final ConfigOption<MemorySize> KV_WRITE_BUFFER_SIZE = key("kv.rocksdb.writebuffer.size")
            .memoryType()
            .defaultValue(MemorySize.parse("64mb"))
    ...
    public static final ConfigOption<Integer> KV_MAX_WRITE_BUFFER_NUMBER = key("kv.rocksdb.writebuffer.count")
            .intType()
            .defaultValue(2)
            .withDescription(
    ...
    public static final ConfigOption<MemorySize> KV_BLOCK_SIZE = key("kv.rocksdb.block.blocksize")
            .memoryType()
            .defaultValue(MemorySize.parse("4kb"))
            .withDescription("The approximate size (in bytes) of user data packed per block. "
                    + "The default blocksize is `4KB`.");

    public static final ConfigOption<MemorySize> KV_METADATA_BLOCK_SIZE = key("kv.rocksdb.block.metadata-blocksize")
            .memoryType()
            .defaultValue(MemorySize.parse("4kb"))
    ...
    public static final ConfigOption<MemorySize> KV_BLOCK_CACHE_SIZE = key("kv.rocksdb.block.cache-size")
            .memoryType()
            .defaultValue(MemorySize.parse("8mb"))
            .withDescription(
                    "The amount of the cache for data blocks in RocksDB. " + "The default block-cache size is `8MB`.");

    public static final ConfigOption<Boolean> KV_USE_BLOOM_FILTER = key("kv.rocksdb.use-bloom-filter")
            .booleanType()
            .defaultValue(true)
            .withDescription("If true, every newly created SST file will contain a Bloom filter. "
                    + "It is enabled by default.");

    public static final ConfigOption<Double> KV_BLOOM_FILTER_BITS_PER_KEY = key("kv.rocksdb.bloom-filter.bits-per-key")
            .doubleType()
            .defaultValue(10.0)
            .withDescription(
    ...
    public static final ConfigOption<MemorySize> KV_RECOVER_LOG_RECORD_BATCH_MAX_SIZE =
            key("kv.recover.log-record-batch.max-size")
                    .memoryType()
                    .defaultValue(MemorySize.parse("16mb"))
                    .withDescription("The max fetch size for fetching log to apply to kv during recovering kv.");

    // ------------------------------------------------------------------------
    //  ConfigOptions for metrics
    // ------------------------------------------------------------------------
    public static final ConfigOption<List<String>> METRICS_REPORTERS = key("metrics.reporters")
            .stringType()
            .asList()
            .noDefaultValue()
            .withDescription("An optional list of reporter names. "
                    + "If configured, only reporters whose name matches in the list will be started");

    public static final ConfigOption<String> METRICS_REPORTER_PROMETHEUS_PORT = key("metrics.reporter.prometheus.port")
            .stringType()
            .defaultValue("9249")
            .withDescription("The port the Prometheus reporter listens on."
    ...
    // ------------------------------------------------------------------------
    public static final ConfigOption<String> METRICS_REPORTER_JMX_HOST = key("metrics.reporter.jmx.port")
            .stringType()
            .noDefaultValue()
            .withDescription("The port for "
    ...
    // ------------------------------------------------------------------------
    public static final ConfigOption<DataLakeFormat> DATALAKE_FORMAT = key("datalake.format")
            .enumType(DataLakeFormat.class)
            .noDefaultValue()
            .withDescription("The datalake format used by Fluss to be as lake storage, such as Paimon, Iceberg, Hudi. "
                    + "Now, only support Paimon.");

    // ------------------------------------------------------------------------
    //  ConfigOptions for fluss kafka
    // ------------------------------------------------------------------------
    public static final ConfigOption<Boolean> KAFKA_ENABLED = key("kafka.enabled")
            .booleanType()
            .defaultValue(false)
            .withDescription("Whether enable fluss kafka. Disabled by default. "
                    + "When this option is set to true, the fluss kafka will be enabled.");

    public static final ConfigOption<List<String>> KAFKA_LISTENER_NAMES = key("kafka.listener.names")
            .stringType()
            .asList()
            .defaultValues("KAFKA")
            .withDescription(
    ...
    public static final ConfigOption<String> KAFKA_DATABASE = key("kafka.database")
            .stringType()
            .defaultValue("kafka")
            .withDescription("The database for fluss kafka. The default database is `kafka`.");

    public static final ConfigOption<Duration> KAFKA_CONNECTION_MAX_IDLE_TIME = key("kafka.connection.max-idle-time")
            .durationType()
            .defaultValue(Duration.ofSeconds(60))
            .withDescription("Close kafka idle connections after the given time specified by this config.");
