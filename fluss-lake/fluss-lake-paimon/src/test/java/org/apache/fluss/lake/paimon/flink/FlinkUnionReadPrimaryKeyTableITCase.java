...
class FlinkUnionReadPrimaryKeyTableITCase extends FlinkUnionReadTestBase {
    ...
    void testUnionReadFullType(Boolean isPartitioned) throws Exception {
        ...
        long tableId = preparePKTableFullType(t1, DEFAULT_BUCKET_NUM, isPartitioned, bucketLogEndOffset);
        ...
        List<String> resultEmptyLog = toSortedRows(batchTEnv.executeSql("select * from " + tableName));
        String expetedResultFromPaimon = buildExpectedResult(isPartitioned, 0, 1);
        assertThat(resultEmptyLog.toString().replace("+U", "+I")).isEqualTo(expetedResultFromPaimon);

        // read paimon directly using $lake
        TableResult tableResult = batchTEnv.executeSql(String.format("select * from %s$lake", tableName));
        List<String> paimonSnapshotRows = CollectionUtil.iteratorToList(tableResult.collect()).stream()
                .map(row -> {
        ...
        assertThat(paimonSnapshotRows.toString().replace("+U", "+I")).isEqualTo(expetedResultFromPaimon);
        ...
        tableResult = batchTEnv.executeSql(String.format("select * from %s where %s", tableName, queryFilterStr));

        List<String> flussPointQueryRows = toSortedRows(tableResult);
        List<String> expectedPointQueryRows = expectedRows.stream()
                .filter(row -> {
        ...
        List<String> paimonPointQueryRows = CollectionUtil.iteratorToList(batchTEnv
                        .executeSql(String.format("select * from %s$lake where %s", tableName, queryFilterStr))
                        .collect())
                .stream()
                .map(row -> {
        ...
        List<String> paimonOptionsRows =
                toSortedRows(batchTEnv.executeSql(String.format("select * from %s$lake$options", tableName)));
        ...
        List<String> projectRows = toSortedRows(batchTEnv.executeSql("select c3, c4 from " + tableName));
        List<Row> expectedProjectRows = expectedRows.stream()
                .map(row -> Row.of(
                        row.getField(2), // c3
                        row.getField(3))) // c4
                .collect(Collectors.toList());
        assertThat(projectRows.toString())
                .isEqualTo(sortedRows(expectedProjectRows).toString());
        // query with project push down
        List<String> projectRows2 = toSortedRows(batchTEnv.executeSql("select c3 from " + tableName));
        List<Row> expectedProjectRows2 = expectedRows.stream()
                .map(row -> Row.of(row.getField(2))) // c3
                .collect(Collectors.toList());
        assertThat(projectRows2.toString())
                .isEqualTo(sortedRows(expectedProjectRows2).toString());
    }
    ...
    private long preparePKTableFullType(
            TablePath tablePath, int bucketNum, boolean isPartitioned, Map<TableBucket, Long> bucketLogEndOffset)
            throws Exception {
    ...
    private Map<TableBucket, Long> getBucketLogEndOffset(long tableId, int bucketNum, Long partitionId) {
    ...
    private String buildExpectedResult(boolean isPartitioned, int record1, int record2) {
        ...
        if (isPartitioned) {
            ...
            return String.format(
                    "[%s, %s]",
                    String.format(records.get(record1), "null"), String.format(records.get(record2), "null"));
        }
    }

    protected long createPkTableFullType(TablePath tablePath, int bucketNum, boolean isPartitioned) throws Exception {
    ...
    private void writeFullTypeRow(TablePath tablePath, String partition) throws Exception {
        List<InternalRow> rows = Collections.singletonList(row(
                true,
                (byte) 100,
                (short) 200,
                30,
