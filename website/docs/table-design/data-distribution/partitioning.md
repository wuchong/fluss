...
- Auto-partitioning rules can only be configured at the time of creating the partitioned table, except `table.auto-partition.num-retention` and `table.auto-partition.num-precreate`, which can be modified by `ALTER TABLE ... SET` or `ALTER TABLE ... RESET` after table creation. For a table with multiple partition keys, `table.auto-partition.num-precreate` must remain `0`.
...
| table.auto-partition.time-format   | String  | no       | based on time unit   | The `DateTimeFormatter` pattern for auto-created partitions. A custom format must list fields from the largest to the smallest time unit, include every field required by `table.auto-partition.time-unit`, and use fixed-width numeric fields. For example, `yyyy-MM-dd` is valid for DAY.                                                                                                                                        |
...
The default format can be overridden with `table.auto-partition.time-format`. Custom formats must satisfy all of the following rules:
