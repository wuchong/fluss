...
When creating a Fluss table, you can specify the number of buckets by setting `'bucket.num' = '<num>'` property for the table, see more details in [DDL](engine-flink/ddl.md). For partitioned tables, `bucket.num` can be altered via `ALTER TABLE SET ('bucket.num' = '<num>')` — the new value applies to newly created partitions while existing partitions retain their original bucket count.
