...
public class ReplicaFetcherITCase {
    ...
    void testProduceLogNeedAck() throws Exception {
        ...
        for (int followId :
            ...
            retry(Duration.ofMinutes(1), () -> assertThat(replicaManager
    ...
    void testPutKvNeedAck() throws Exception {
        ...
        long tableId = createTable(FLUSS_CLUSTER_EXTENSION, DATA1_TABLE_PATH_PK, createPkTableDescriptor());
        int bucketId = 0;
        TableBucket tb = new TableBucket(tableId, bucketId);
        ...
        assertPutKvResponse(
                leaderGateWay
                        .putKv(newPutKvRequest(tableId, bucketId, -1, genKvRecordBatch(DATA_1_WITH_KEY_AND_VALUE)))
                        .get(),
                bucketId);
        ...
        for (int followId :
            ...
            LogRecords records = resultForBucket.records();
            assertThat(records).isNotNull();
            assertLogRecordsEqualsWithRowKind(DATA1_ROW_TYPE, records, EXPECTED_LOG_RESULTS_FOR_DATA_1_WITH_PK);
        }
    }
    ...
    void testFlushForPutKvNeedAck() throws Exception {
        ...
        int followerToStop = FLUSS_CLUSTER_EXTENSION.getTabletServerNodes().stream()
        ...
        retry(Duration.ofMinutes(1), () -> assertThat(replica.getLocalLogEndOffset())
                .isEqualTo(4L));
        ...
        for (Tuple2<byte[], byte[]> keyValue : expectedKeyValues) {
            assertLookupResponse(
                    leaderGateWay
                            .lookup(newLookupRequest(tableId, bucketId, keyValue.f0))
                            .get(),
                    null);
        }
